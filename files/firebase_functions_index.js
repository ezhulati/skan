...
app.get('/venue/:slug/tables', async (req, res) => {
  try {
    const { slug } = req.params;
    ...
    const tables = tablesSnapshot.docs.map(doc => ({
      ...
      qrUrl: `https://skan.al/order/${slug}/${doc.data().tableNumber}`
    }));
...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    // For MVP, we'll use Firebase Auth
...
app.get('/', (req, res) => {
  res.json({ 
    message: 'Skan.al API is running',
    version: '1.0.0',
    endpoints: [
      'GET /venue/:slug/menu',
      'GET /venue/:slug/tables', 
      'POST /orders',
...
// Optional: Real-time order notifications (can be added later)
exports.onOrderCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    // Here you could send notifications to restaurant staff
...
exports.onOrderStatusChanged = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (before.status !== after.status) {
      ...
      // Here you could notify the customer of status changes
