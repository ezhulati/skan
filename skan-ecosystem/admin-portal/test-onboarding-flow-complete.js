...
async function testCompleteOnboardingFlow() {
  ...
  try {
    ...
    // Test 2: Wait for onboarding wizard to load (use correct selector)
    ...
    // Navigate back to onboarding to test mobile view
    await page.goto('http://localhost:3000/onboarding', { waitUntil: 'networkidle0' });
    await new Promise(resolve => setTimeout(resolve(2000);
    
    const mobileWizard = await page.$('.onboarding-wizard');
    addTestResult('Mobile view loads correctly', mobileWizard !== null);

    // Test 16: Albanian translations validation
    console.log('\n📋 TEST 16: Albanian Translations Validation');
